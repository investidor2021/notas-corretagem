...
def parse_br_float(num_str: str) -> float:
    """
    Converte uma string numérica no formato brasileiro (ex: "1.234,56")
    para um float (ex: 1234.56), de forma segura.
    """
    if isinstance(num_str, (int, float)):
        return float(num_str)

    if not isinstance(num_str, str):
        return 0.0 # Retorna 0 se não for um tipo de texto esperado

    # Remove espaços em branco das extremidades
    num_str = num_str.strip()
    
    # Remove o separador de milhares '.'
    num_str_limpo = num_str.replace('.', '')
    
    # Troca o separador decimal ',' por '.'
    num_str_limpo = num_str_limpo.replace(',', '.')
    
    try:
        # Tenta converter o resultado para float
        return float(num_str_limpo)
    except (ValueError, TypeError):
        # Se a conversão falhar (ex: string vazia ou texto não numérico),
        # retorna 0.0 para evitar que o programa quebre.
        return 0.0

...
class GenericParser(BaseParser):
    """
    Parser genérico que utiliza as regras de extração originais.
    Serve como fallback quando um parser específico da corretora não é encontrado.
    """
    NOME_CORRETORA = "Genérico"

    def extrair_info_cabecalho(self) -> dict:
        """Extrai informações do cabeçalho da nota."""
        info = {
            "numero_nota": "N/A",
            "folha": "N/A",
            "data_pregao": "N/A",
        ...
        # Tenta encontrar o nome e CNPJ da corretora
        ...
        # Regex para Nr. nota, Folha, Data pregão
    ...
    def extrair_resumo(self) -> pd.DataFrame:
        """Extrai a tabela de resumo financeiro da nota."""
        resumos = []
        ...
        if match:
            ...
            # Adiciona a linha final "Valor das operações"
            resumos.append({
                "Numero Nota": self.info_cabecalho.get('numero_nota'),
                "Data Pregao": self.info_cabecalho.get('data_pregao'),
        ...
        if not df.empty:
            ...
            # Remover duplicatas que podem surgir de múltiplas páginas
